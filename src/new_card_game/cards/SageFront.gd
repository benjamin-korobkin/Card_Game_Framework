...
func _ready() -> void:
	...
	card_label_min_sizes["Era"] = Vector2(CFConst.CARD_SIZE.x - 4,20)
	card_labels["Teaching"] = find_node("Teaching")
	card_label_min_sizes["Teaching"] = Vector2(CFConst.CARD_SIZE.x, 36)
	card_labels["Power"] = find_node("Power")
	card_label_min_sizes["Power"] = Vector2(CFConst.CARD_SIZE.x - 4,30)
